...
class CF:
    def __init__(self,
        ...
        for i in range(len(self.t)):
            ...
            self.cov_acc[i, :] = np.ones(
                [1, 3]) / np.sum(np.ones([cnt, 3]) /
                                 np.square(var[idx, 0:3]), 0)
            self.cov_omg[i, :] = np.ones(
                [1, 3]) / np.sum(np.ones([cnt, 3]) /
                                 np.square(var[idx, 3:6]), 0)

            self.acc[i, :] = np.sum(
                acc[idx, :] / np.square(var[idx, 0:3]), 0) * self.cov_acc[i, :]
            self.omg[i, :] = np.sum(
                omg[idx, :] / np.square(var[idx, 3:6]), 0) * self.cov_omg[i, :]
